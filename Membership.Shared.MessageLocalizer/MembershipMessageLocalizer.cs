...
{
    readonly Dictionary<string, string> MessagesSpanish = new()
    {
        ...
        {MessageKeys.PasswordTooShortErrorMessage, "Se requiere al menos 6 caracteres en la contraseña." },
        ...
        {MessageKeys.UnableToGetExternalUserTokens , "No se pudo realizar la operación solicitada."  },
        {MessageKeys.InvalidStateValue , "Se recibió un valor 'state' incorrecto."  },
        {MessageKeys.MissingAuthorizeCallbackParameters, "No se recibieron los parámetros esperados."  },
        {MessageKeys.InvalidNonceValue, "El valor 'nonce' no es válido."  }
    };
    ...
    public string this[string key]
    {
        get
        {
            MessagesSpanish.TryGetValue(key, out string message);
